...
import { ChevronLeft, ChevronRight, X, ExternalLink, Stethoscope, Ear, Baby, Activity, FolderGit2, Construction } from 'lucide-react'
import { useI18n } from '../i18n'

const projectIcons = {
  'Hearing Evaluations':        Stethoscope,
  'Hearing Aid Fitting':        Ear,
  'Pediatric Hearing Program':  Baby,
  'Tinnitus & Balance Care':    Activity,
}
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
          <div style={{ width: 52, height: 52, borderRadius: 12, flexShrink: 0, background: c.bg, border: `1px solid ${c.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <ProjIcon name={proj.name} size={26} color={c.accent} />
          </div>
          <div>
...
const FILTERS = [
  { key: 'all',       label: 'All'           },
  { key: 'adult',     label: 'Adult Care'    },
  { key: 'pediatric', label: 'Pediatric Care' },
]
...
export default function Projects() {
  ...
  // Reset scroll position & arrows whenever the filter changes
  useEffect(() => {
    const el = scrollRef.current
    if (!el) return
    el.scrollTo({ left: 0 })
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: `0 ${sidePad}px` }}>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45 }}
          ...
          <div style={{ marginTop: 16, height: 2, background: 'var(--border)', borderRadius: 2, position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: `${100 / Math.max(filtered.length, 1)}%`, background: 'linear-gradient(90deg, var(--gradient-from), var(--gradient-to))', borderRadius: 2 }}/>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14, gap: 8, flexShrink: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0, flex: 1 }}>
                      ...
                      <div style={{ minWidth: 0 }}>
                        <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)', letterSpacing: '-0.02em', marginBottom: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{proj.name}</h3>
                        <span style={{ display: 'block', fontSize: 11.5, color: c.accent, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{proj.subtitle}</span>
                      </div>
                    </div>
                  ...
                  <div style={{ marginTop: 16, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {/* Live links — or an "under construction" badge if none */}
                    {proj.links?.length > 0 ? (
