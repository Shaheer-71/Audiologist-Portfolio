...
import { ChevronLeft, ChevronRight, Stethoscope, Ear, Baby, HeartHandshake } from 'lucide-react'
import { skills } from '../data/portfolio'

const categoryIcons = {
  'Diagnostic Testing': Stethoscope,
  'Hearing Aid Care':   Ear,
  'Pediatric Care':     Baby,
  'Patient Care':       HeartHandshake,
}
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45 }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginTop: 8 }}>
            <h2 style={{ fontSize: 'clamp(28px, 4.5vw, 46px)', fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.02em', lineHeight: 1.1 }}>
              Skills &amp; <span className="text-gradient">Specialties</span>
            </h2>
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 22, flexShrink: 0 }}>
                    <div style={{ width: 42, height: 42, borderRadius: 10, background: c.bg, border: `1px solid ${c.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {(() => { const Icon = categoryIcons[cat.category] || Stethoscope; return <Icon size={20} strokeWidth={1.75} style={{ color: c.accent }} /> })()}
                    </div>
