...
const from = () => `"${personal.name} Portfolio" <${process.env.EMAIL_USER}>`
const owner = () => process.env.OWNER_EMAIL || personal.email
      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;padding:24px;background:#0d0d1f;color:#e2e8f0;border-radius:12px">
          ...
          <p style="color:#a1a1aa;line-height:1.7">Best regards,<br>
            <strong style="color:#f4f4f5">${personal.name}</strong><br>
            <span style="color:#8b5cf6;font-size:13px">${personal.title}</span>
          </p>
        </div>`,
    }),
  ])
...
export async function sendScheduleEmails({ name, email, date, time, purpose, eventLink, meetLink }) {
  ...
  await t.sendMail({
    from: from(),
    to: owner(),
    subject: `🗓️ Appointment Scheduled: ${name} — ${formatted} at ${time}`,
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;padding:24px;background:#0d0d1f;color:#e2e8f0;border-radius:12px">
        <h2 style="color:#8b5cf6;margin:0 0 20px">Appointment Scheduled!</h2>
        <table style="width:100%;border-collapse:collapse;margin-bottom:24px">
          ...
          <tr><td style="padding:8px 0;color:#a1a1aa">Time</td><td style="color:#f4f4f5">${time}</td></tr>
          <tr><td style="padding:8px 0;color:#a1a1aa">Reason</td><td style="color:#f4f4f5">${purpose || 'General visit'}</td></tr>
        </table>
